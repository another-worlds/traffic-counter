...
# Responsive layout: current folder dashboard first, selector/list second
sel_col, videos_col = st.columns([1, 2])
with sel_col:
...
st.markdown(
    f"<div style='padding:0.75rem 1rem;border:1px solid #e2e8f0;border-left:7px solid {accent};"
    f"background:linear-gradient(180deg,#ffffff 0%,#f8fafc 100%);border-radius:12px;margin-bottom:0.5rem;'>"
    f"<strong style='font-size:1.05rem'>{status_icon} {selected_row['folder']}</strong>"
    f"<br/><span style='color:#334155;'>Videos: {selected_row['count']} · "
...
for row in folder_rows:
    icon, row_color = _health_badge(row["completion"])
    selected_tag = "<span style='font-size:0.8rem;background:#dbeafe;color:#1d4ed8;padding:0.1rem 0.45rem;border-radius:999px;'>selected</span>" if row["folder"] == sel_folder else ""
    st.markdown(
        f"<div style='padding:0.55rem 0.7rem;border:1px solid #e2e8f0;border-radius:10px;margin:0.35rem 0;"
        f"background:#ffffff;'>"
        f"<div style='display:flex;justify-content:space-between;align-items:center;gap:0.5rem;'>"
        f"<div><strong>{icon} {row['folder']}</strong></div><div>{selected_tag}</div></div>"
        f"<div style='color:#475569;margin-top:0.2rem;'>"
        f"{row['analyzed']}/{row['count']} complete · {row['processed_hours']:.2f}h / {row['total_hours']:.2f}h · errors: {row['errors']}"
        f"</div>"
        f"<div style='height:7px;background:#e2e8f0;border-radius:999px;margin-top:0.35rem;'>"
        f"<div style='height:7px;width:{row['completion']*100:.1f}%;background:{row_color};border-radius:999px;'></div>"
        "</div></div>",
        unsafe_allow_html=True,
    )
